...
class DHF1KDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.len_snippet):
            img = Image.open(os.path.join(path_clip, '%05d.jpg' % (start_idx + self.alternate * i + 1))).convert('RGB')
            sz = img.size

            if self.mode != "save":
                gt = np.array(
                    Image.open(os.path.join(path_annt, '%05d.jpg' % (start_idx + self.alternate * i + 1))).convert('L'))
                gt = gt.astype('float')
